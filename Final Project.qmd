...
infant_mortality <- read_csv2(
 ...
 mutate(
    ...
    `Décédés de moins de 7 jours (b)` = as.numeric(gsub(" ", "", `Décédés de moins de 7 jours (b)`)),
    `Décédés de moins de 28 jours (b)` = as.numeric(gsub(" ", "", `Décédés de moins de 28 jours (b)`)),
    `Infantile (c)` = as.numeric(gsub(",", ".", `Infantile (c)`)),
    `Néonatale (d)` = as.numeric(gsub(",", ".", `Néonatale (d)`))
  ) %>%
...
All columns were initially imported as character type due to formatting issues so we converted the mortality count variable to numeric using `as.numeric()`, then we removed spaces from numbers (e.g., "27 373" → "27373") using `gsub(" ", "", ...)`.
