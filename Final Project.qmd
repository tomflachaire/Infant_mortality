...
get_data_info <- function(file_path) {
  ...
  list(
    ...
    Lignes = nrow(data),
    Colonnes = ncol(data),
    Noms_des_colonnes = paste(colnames(data), collapse = ", ")
  )
}

data_files <- list.files(path = "data/", pattern = "\\.csv$", full.names = TRUE)
data_info <- lapply(data_files, get_data_info) 
data_info_df <- do.call(rbind.data.frame, data_info) 
...
#We are deleting the ligns with no values.
...
```{r}
# To ilustrate our infant mortality variable, we will show the difference between top 5 and bottom 5 departements. Showing all 101 departement in one graph would be ugly.
